pipeline {
    ...
    environment {
        DOCKER_HUB_REPO = 'areysid/techsolutions-app'
        K8S_CLUSTER_NAME = 'sid-ingress-cluster'
    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git 'https://github.com/areysid/Microservices-ingress-controller.git'
            }
        }
        ...
        stage('Deploy to Kubernetes') {
            steps {
                ...
                script {
                    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-creds']]) {
                        sh "sed -i 's|areysid/techsolutions-app:latest|areysid/techsolutions-app:${env.IMAGE_TAG}|g' k8s/deployment.yaml"
